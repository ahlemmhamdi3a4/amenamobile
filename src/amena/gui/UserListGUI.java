...
public class UserListGUI extends SideMenuBaseForm {
private EncodedImage palceHolder;
  ...
  private Container createPaginationContainer(Resources res) {
    ...
    public UserListGUI(Form previous,Resources res,User user) {
        /*current = this;
...
//         AnnonceService as = new AnnonceService();
        if (!UserService.getInstance().getAllUsers().isEmpty()) {
            Container con3=new Container(BoxLayout.y());
         for (User a : UserService.getInstance().getAllUsers()) {
                
                int id=a.getId();
                String titre=a.getNom();
                String type=a.getRoles();
                String categorie=a.getEmail();
//                String date=a.getDate_n().toString();
...
//                String ress = dateFormat.format(a.getDate_n());
...
//                    imgv=new ImageViewer(res.getImage("noimagefound.jpg").scaled(120, 100));
                ...
                con1.addAll(lTitre,lCategorie,lType);
